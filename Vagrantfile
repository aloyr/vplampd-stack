...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  settings['shares'].each do |item|
    if item['vm'] == settings['webroot']
      ...
      #temporarily revert to vboxfs for amazee meeting
  ...
  config.vm.provision :shell, :inline => "if [ ! `rpm -q puppet | grep -E '^puppet-3'` ]; then echo search home.local > /etc/resolv.conf; echo 'nameserver #{dnsServer}' >> /etc/resolv.conf; yum update -y puppet; yum update -y rubygem-json; fi"
